...
export class CadastroProduto {
  ...
  nomeDigitado: string = '';
  marcaDigitada: string = ''; // <--- ADICIONADO AQUI
  ...
  executarCadastroProduto(): void {
    ...
    // Se o seu serviço aceitar a marca, passe ela aqui. Caso contrário, mantenha apenas os parâmetros do DTO.
    this.cadastroProdutoService.cadastroProduto(
      this.nomeDigitado, 
      this.codBarraDigitada, 
      this.quantidadeDigitada, 
      this.precoCustoDigitado, 
      this.precoVendaDigitado, 
      usuarioAtual
    ).subscribe({
      next: (resposta) => {
      ...
      error: (error) => {
        ...
        setTimeout(() => {
          alert('Cadastro do produto não foi possível');
        }, 50);
      }
    });
  }
